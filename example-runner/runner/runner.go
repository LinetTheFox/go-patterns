...
type Runner struct {
	...
	// asynchronously in index order
	tasks []func(int)
}

// ErrTimeout is returned when an event is received on the timeout
...
func (r *Runner) Start() error {
	...
	// Run different tasks on a different goroutine
	go func() {
		r.complete <- r.run()
	}()
...
func (r *Runner) gotInterrupt() bool {
	select {
	// Signalled when an interrupt even is sent
	case <-r.interrupt:
